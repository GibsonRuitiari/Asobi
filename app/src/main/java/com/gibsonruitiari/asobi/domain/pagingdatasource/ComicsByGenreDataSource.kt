...
class ComicsByGenreDataSource (private val logger: Logger):BaseDataSource<SManga>(logger){
    private val genre= MutableStateFlow(Genres.DC_COMICS)
    fun setGenre(genres: Genres){
        // if the genres is equal to genre.value then it won't change hence saving us from re-fetching data
        // unnecessarily
        genre.value = genres
        logger.i("new genre set ${genre.value}")
    }
    override suspend fun loadData(page: Int): List<SManga> {
      return comicsByGenre(page,genre.value).firstOrNull()?.mangas ?: emptyList()
    }
}